...
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const latestTransactionsPromise = Order.find({})
            .select(["status", "orderItems", "discount", "total"])
            .limit(4);
        const [thisMonthProducts, thisMonthUsers, thisMonthOrders, lastMonthProducts, lastMonthUsers, lastMonthOrders, productsCount, usersCount, allOrders, lastSixMonthOrders, categories, femaleUsers, latestTransactions,] = await Promise.all([
        ...
        const count = {
            revenue: allOrders.reduce((total, order) => total + (order.total || 0), 0),
            user: usersCount,
...
export const getPieCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const [processingOrder, shippedOrder, deliveredOrder, categories, productsCount, productOutOfStock, allOrders, allUsers, adminUsers, customerUsers,] = await Promise.all([
        ...
        const orderFullfillmentInfo = {
            processingOrder: processingOrder,
            shippedOrder: shippedOrder,
            deliveredOrder: deliveredOrder,
        };
        const productCategories = await getInvetories({
            categories,
            productsCount,
        });
        ...
        const netMargin = grossIncome - discount - marketingCost - productionCost - burnt;
